...
class LoRATrainer():
    ...
    def train(self):
        ...
        checkpoint_dir = "../saves/model_checkpoints"
        ...
        while train_steps < self.target_steps:
            for batch_input_ids, batch_target_ids in train_loader:
                ...
                # Save checkpoint based on frequency (every 5 steps in this case)
                if train_steps % 5 == 0:
                ...
                with torch.no_grad(): ### maybe - only validate on last batch (to save compute)
                    for b_n, (batch_input_ids, batch_target_ids) in enumerate(val_loader): ### LIMIT: val-limit - val-batches
        ...
        final_model_path = os.path.join('../saves', "final_model.pt")
